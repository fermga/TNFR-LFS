...
* ``ackermann_parallel_index`` and ``slide_catch_budget`` come exclusively from the ``slip_angle_*`` channels and ``yaw_rate`` emitted by OutSim; without that telemetry the Ackermann and slide-recovery budgets remain zero and reports show the literal ``"sin datos"`` (“no data”) token.
