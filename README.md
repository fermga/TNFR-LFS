...
- **Ventilación / fade de freno** – Live for Speed no expone temperaturas de freno en la telemetría pública. Cuando OutGauge transmite el bloque extendido OG\_EXT\_BRAKE, TNFR × LFS respeta esos datos; si no, el fusionador integra un proxy térmico basado en carga, deceleración y velocidad para mantener activas las métricas de ventilación y *fade* sin romper la canonicidad del flujo.【F:tnfr_lfs/acquisition/fusion.py†L70-L173】【F:tnfr_lfs/acquisition/fusion.py†L248-L338】【F:tnfr_lfs/analysis/brake_thermal.py†L1-L88】【F:tnfr_lfs/core/metrics.py†L1101-L1250】

#### Proxy térmico de frenos

Cuando OutGauge no entrega `brake_temps`, TNFR × LFS recurre a un estimador convectivo sencillo que integra `Q_in = m·a·v·η` ponderado por carga instantánea de cada rueda y resta la disipación proporcional a la velocidad del aire. El modelo opera dentro de `[T_amb, 1200 °C]` y se reinicia automáticamente al cambiar de coche/pista o al restablecer la fusión.【F:tnfr_lfs/analysis/brake_thermal.py†L10-L88】【F:tnfr_lfs/acquisition/fusion.py†L70-L173】

Las constantes del proxy se pueden ajustar en `config/global.toml` (bloque `[thermal.brakes]`) o por coche en `data/cars/*.toml`; el pack distribuido define valores por defecto en `tnfr_lfs/data/fusion_calibration.toml`. Asimismo, la variable de entorno `TNFR_LFS_BRAKE_THERMAL` acepta los modos `auto`, `off` y `force` para alternar entre telemetría real, proxy o estimación obligatoria, de modo que las sesiones reproducibles puedan fijar una única semántica.【F:config/global.toml†L17-L24】【F:tnfr_lfs/data/fusion_calibration.toml†L1-L40】【F:tnfr_lfs/acquisition/fusion.py†L70-L173】
